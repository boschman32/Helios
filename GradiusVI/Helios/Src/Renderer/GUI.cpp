...
#include "../Shaders/Headers/ImGUIShader_vs.h"
#include "../Shaders/Headers/ImGUIShader_ps.h"
...
bool GUI::Initialize()
{
    ...
    if (!ImGui_ImplWin32_Init(m_rWindow.GetWindowHandle()))
    {
        return false;
    }
    ...
    pipelineStateStream.VS = { g_ImGUIShader_vs, sizeof(g_ImGUIShader_vs) };
    pipelineStateStream.PS = { g_ImGUIShader_ps, sizeof(g_ImGUIShader_ps) };
...
//    DirectX::XMMATRIX projectionMatrix = DirectX::XMMatrixOrthographicRH( drawData->DisplaySize.x, drawData->DisplaySize.y, 0.0f, 1.0f );
    ...
    float R = drawData->DisplayPos.x + (drawData->DisplaySize.x * io.DisplayFramebufferScale.x);
    float T = drawData->DisplayPos.y;
    float B = drawData->DisplayPos.y + (drawData->DisplaySize.y * io.DisplayFramebufferScale.y);
    float mvp[4][4] =
    {
