...
void ResourceStateTracker::TransitionResource(const DX12Resource& resource, D3D12_RESOURCE_STATES stateAfter, UINT subResource)
{
    TransitionResource(resource.GetD3D12Resource().Get(), stateAfter, subResource);
}

void ResourceStateTracker::UAVBarrier(const DX12Resource* resource)
{
...
void ResourceStateTracker::AliasBarrier(const DX12Resource* resourceBefore, const DX12Resource* resourceAfter)
{
