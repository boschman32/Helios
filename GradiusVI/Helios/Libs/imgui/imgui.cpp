...
ImVec2 ImGui::GetPixelSizeFromRelativeSize(ImVec2& size)
{
    ImGuiIO& io = ImGui::GetIO();
    ImVec2 displaySize = io.DisplaySize;
    ImVec2 pixelSize;
    pixelSize.x = size.x * displaySize.x;
    pixelSize.y = size.x * displaySize.y;
	return pixelSize;
}
...
void ImGui::SetNextWindowPosNDC(const ImVec2& pos, ImGuiCond cond, const ImVec2& pivot)
{
    ImGuiContext& g = *GImGui;
    ImGuiIO& io = ImGui::GetIO();
    ImVec2 displaySize = io.DisplaySize;
    float xDivided = displaySize.x / 2;
    float yDivided = displaySize.y / 2;
    ImVec2 correctPosition;
    correctPosition.x = xDivided + (pos.x * xDivided);
    correctPosition.y = yDivided + (pos.y * yDivided);
    IM_ASSERT(cond == 0 || ImIsPowerOfTwo(cond)); // Make sure the user doesn't attempt to combine multiple condition flags.
    g.NextWindowData.Flags |= ImGuiNextWindowDataFlags_HasPos;
    g.NextWindowData.PosVal = correctPosition;
    g.NextWindowData.PosPivotVal = pivot;
    g.NextWindowData.PosCond = cond ? cond : ImGuiCond_Always;
}
...
IMGUI_API void ImGui::SetNextWindowSizeRelative(const ImVec2& size, ImGuiCond cond)
{
    ImGuiContext& g = *GImGui;
    IM_ASSERT(cond == 0 || ImIsPowerOfTwo(cond)); // Make sure the user doesn't attempt to combine multiple condition flags.
    g.NextWindowData.Flags |= ImGuiNextWindowDataFlags_HasSize;
    ...
    ImGuiIO& io = ImGui::GetIO();
    ImVec2 displaySize = io.DisplaySize;
    newSize.x = size.x * displaySize.x;
    newSize.y = size.y * displaySize.y;
    g.NextWindowData.SizeVal = newSize;
    g.NextWindowData.SizeCond = cond ? cond : ImGuiCond_Always;
}
