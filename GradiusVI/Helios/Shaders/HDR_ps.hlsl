...
float4 main( PixelShaderInput IN ) : SV_Target
{
    ...
    float4 texColor = DiffuseTexture.SampleLevel( LinearRepeatSampler, IN.TexCoord, 0 );

    float4 outColor = (emissive + ambient + diffuse + specular) * texColor;
    outColor = float4(outColor.x, outColor.y, outColor.z, texColor.w);
