...
{
public:
    explicit DX12Resource(const std::wstring& name = L"");
    explicit DX12Resource(const D3D12_RESOURCE_DESC& resourceDesc,
        const D3D12_CLEAR_VALUE* clearValue = nullptr,
        const std::wstring& name = L"");
    explicit DX12Resource(Microsoft::WRL::ComPtr<ID3D12Resource> resource, const std::wstring& name = L"");

    DX12Resource(const DX12Resource& copy);
    DX12Resource(DX12Resource&& copy);

    DX12Resource& operator=(const DX12Resource& other);
    DX12Resource& operator=(DX12Resource&& other) noexcept;
