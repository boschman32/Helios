...
namespace Gradius
{
    BigCoreBullet::BigCoreBullet(Helios::Entity& a_owner, const Helios::ComponentID& a_id)
        : BaseBullet(a_owner, a_id)
    {
        
    }

    void BigCoreBullet::Start()
    {
        m_collider = m_owner->GetComponent<Helios::QuadCollider>();
        m_collider->SetTag(Tags::m_bulletTag);
    }

    void BigCoreBullet::Update(float deltaTime)
    {
        Move(deltaTime);
        CheckBounds();
    }

    void BigCoreBullet::OnOverlapBegin(Helios::Collider* a_collider)
    {
    ...
    void BigCoreBullet::Move(float deltaTime)
    {
    ...
    void BigCoreBullet::CheckBounds()
    {
